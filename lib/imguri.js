...
var validate = {
  ...
  local: function (item, options, callback) {
    fs.exists(item, function (exists) {
      if (!exists) return callback(new Error('no such file: ' + item));

      fs.stat(item, function (err, stats) {
        if (!err && stats) {
          if (stats.size > SIZE_LIMIT && !options.force) {
            callback(new Error('Size limit exceeded: ' + stats.size + ' > ' + SIZE_LIMIT + ' Set options.force to override'));
          }
          // go ahead
          callback(null);
        } else {
  ...
  network: function (item, options, callback) {
    request.head(item, function (err, res) {
      if (!err && res && res.headers) {
        if (!(/image/gi).test(res.headers['content-type'])) {
          return callback(new Error('Not an image, content-type: ' + res.headers['content-type'] + ', link: ' + item));
        }
        ...
        if (length && !isNaN(length) && (length < SIZE_LIMIT || options.force)) {
          ...
          callback(new Error('Size limit exceeded: ' + length + ' > ' + SIZE_LIMIT + ' Set options.force to override'));
        }
      }
    });
  }
